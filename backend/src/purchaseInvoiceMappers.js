...
function fromRow(row) {
  ...
  return {
    ...
    freightPaymentSource: (row.freight_payment_source || row.freightPaymentSource || 'caixa'),
...
function toRow(invoice) {
  ...
  return {
    ...
    address: inv.address || '',
    purchase_account_code: inv.purchaseAccountCode || inv.purchase_account_code || '212',
    iva_account_code: inv.ivaAccountCode || inv.iva_account_code || '3451',
    ...
    freight_cost: Number(inv.freightCost ?? inv.freight_cost ?? 0),
    freight_other_costs: Number(inv.freightOtherCosts ?? inv.freight_other_costs ?? 0),
    freight_source_account: inv.freightSourceAccount || inv.freight_source_account || '',
    freight_source_name: inv.freightSourceName || inv.freight_source_name || '',
    freight_payment_source: inv.freightPaymentSource || inv.freight_payment_source || 'caixa',
    freight_caixa_id: inv.freightCaixaId || inv.freight_caixa_id || null,
    freight_bank_account_id: inv.freightBankAccountId || inv.freight_bank_account_id || null,
