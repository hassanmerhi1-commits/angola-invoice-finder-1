...
function rowScore(row: Product, branchId?: string, catalogBranchIds: string[] = []): number {
  const stock = readProductStock(row);
  const rowBranch = String(row.branchId || '').trim();
  ...
  if (branchId) {
    // Filial scope: prefer shared catalog (NULL) over sede rows — sede products.stock is not filial qty.
    let score = stock;
    if (!rowBranch) score += 500_000;
    else if (!isCatalogBranchScope(rowBranch, catalogBranchIds)) score += 50_000;
  ...
  if (normalizeProductBranchScopeKey(row.branchId, catalogBranchIds) === 'catalog' && !rowBranch) {
    score += 500;
  }
