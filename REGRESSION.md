...
## Inventory branch scope (Sede / regional hubs)

- Picking a specific branch in the Inventory toolbar — including one whose name/code matches the "head office" heuristic (`is_main`, code `MAIN`/`SEDE*`, or name containing "sede", e.g. a regional hub like "Sede Soyo") — must show **that branch's own** stock, never the company-wide consolidated total. Only the explicit **"All branches (total stock)"** row (`BranchSelector` with `includeAllBranches`) triggers the consolidated grid. `useInventoryBranchScope`'s `isInventoryConsolidated` must stay `inventoryScopeId === ALL_BRANCHES_SCOPE_ID`, not the broader `isConsolidatedBranchScope` used by other pages (expenses/payments), or a Sede-named branch's own inventory intermittently renders a blended/other-branch view instead of its own.
- The "All branches" choice is Inventory-local (`kwanza_inventory_scope_id`), not pushed to the global top-nav scope — other pages keep operating against a real branch.
- Don't force-resync `inventoryScopeId` to the global top-nav branch on every unrelated re-render (branches list refresh, etc.) — only when the global scope itself actually changed — or an explicit local "All branches" pick gets silently reverted moments later.
- `ensureTreasuryRegistersFromCoa` (caixa.js) must not unconditionally overwrite `branches.is_main` on every sync/startup based on a non-deterministic `name ILIKE '%sede%'` tiebreak when multiple branches match — only assign `is_main` when no branch already has it, and break ties deterministically (oldest branch first). Flapping `is_main` cascades into branch-switch permissions and catalog dedup for whichever branch loses the flag.
